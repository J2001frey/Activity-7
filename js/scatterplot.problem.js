                      ...
                      margin = 50)
{
    ...
    const color = d3.scaleOrdinal()
        ...
        .range(d3.schemeTableau10) // color scheme of tableau10  KDJBXZMZNX
    ...
    Fig.selectAll('.markers')
        ...
        .append('circle')
        // We set the class of the circle to include the category name (e.g., "USA")
    ...
    function brushStart() {
        ...
        if (d3.brushSelection(this) === null || d3.brushSelection(this)[0][0] === d3.brushSelection(this)[1][0]) {
            ////////////////////////////////////////////////////////////////////////
            // Missing Part 4
            ////////////////////////////////////////////////////////////////////////
    ...
    function brushed() {
        ...
        // Inverse mapping from screen coordinates to data values
        ...
        d3.selectAll("circle").classed("selected", (d, i) => {
            // Check visibility based on opacity set by the legend click handler
            const isVisible = d3.select(document.getElementById(`id_${i} ${d[colorCol]}`)).style("opacity") > 0.2; // Opacity check is against the faded state

            if (isVisible && +d[xCol] >= X1 && +d[xCol] <= X2 && +d[yCol] <= Y1 && +d[yCol] >= Y2) {
                return true;
            }
            return false;
        });
    ...
    legends_items.append("rect")
        ...
        .attr("width","40")
        .attr("height","40")
        // Set the class of the rect to the category name for global selection
        .attr("class",d=>d)
    ////////////////////////////////////////////////////////////////////////
    // Missing Part 5 - Toggles visibility to 0.2 (faded) and syncs legend state across all graphs
    ////////////////////////////////////////////////////////////////////////
        .on("click", function(event, d) {
            // Determine the new state based on the *current* state of the clicked rect
            const isCurrentlyDisabled = d3.select(this).classed("disabled");
            const newStateDisabled = !isCurrentlyDisabled; 

            // 1. Toggle the 'disabled' class on ALL legend rectangles corresponding to this category (d)
            d3.selectAll(`rect.${d}`)
                .classed("disabled", newStateDisabled);

            // 2. Toggle the opacity/visibility of ALL circles corresponding to this category (d) across the entire document.
            d3.selectAll(`circle.${d}`)
                .transition()
                .duration(200)
                // Set opacity to 0.2 (fade) if disabling, or 1 (reappear) if re-enabling
                .style("opacity", newStateDisabled ? 0.2 : 1); 

            // 3. Clear brush selection styling if the category is being faded (across the entire document).
            if (newStateDisabled) {
                d3.selectAll(`circle.${d}`).classed("selected", false);
            }
        })
    ////////////////////////////////////////////////////////////////////////
    legends_items
